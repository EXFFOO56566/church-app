	...
	public function __construct(){
		...
		public function makecomment(){
			...
			if(!empty($data)){
				  ...
				  if($email!="" && $media !="" && $content != ""){
						 $comment = $this->comments_model->makeComment($media,$email,$content);
						 $total_count = $this->comments_model->get_total_comments($media);
				  }
			 }
		...
		function loadcomments(){
          ...
          $media = $data->media;
				...
				$this->load->model('comments_model');
				$results = $this->comments_model->loadcomments($media,$id);
				$has_more = $this->comments_model->checkIfMediaHasMoreComments($media,$id);
				$total_count = $this->comments_model->get_total_comments($media);
