...
class Postreplies extends BaseController {
	...
	public function __construct(){
        parent::__construct();
        $this->load->model('postreplies_model');
    }
		...
		public function replycomment(){
			...
			if(!empty($data)){
				  ...
				  if($email!="" || $media !="" || $content != ""){
						 $reply = $this->postreplies_model->replyComment($comment,$email,$content);
						 $total_count = $this->postreplies_model->get_total_replies($comment);
				  }
			 }
			 echo json_encode(array("status" => $this->postreplies_model->status,"message" => $this->postreplies_model->message,
		...
		public function editreply(){
			...
			if(!empty($data)){
				  ...
				  if($content != "" || $id != ""){
						 $comment = $this->postreplies_model->editReply($id,$content);
				  }
			 }
			 echo json_encode(array("status" => $this->postreplies_model->status,"message" => $this->postreplies_model->message,
		 "comment" => $comment));
		...
		public function deletereply(){
			...
			if(!empty($data)){
				  ...
				  if($id != ""){
						 $this->postreplies_model->deleteReply($id);
						 $total_count = $this->postreplies_model->get_total_replies($comment_id);
				  }
			 }
			 echo json_encode(array("status" => $this->postreplies_model->status,"message" => $this->postreplies_model->message, "total_count" => $total_count));
		}
		...
		function loadreplies(){
          ...
          $comment = $data->comment;
				...
				$this->load->model('postreplies_model');
				$results = $this->postreplies_model->loadreplies($comment,$id);
				$has_more = $this->postreplies_model->checkIfCommentHaveMoreReplies($comment,$id);
				$total_count = $this->postreplies_model->get_total_replies($comment);
